% program 
% D1_dyn_ND_T_with_prod_explicit_euler_dr
% problem: a*dT/dt = k*d^2T/dx^2+s(x),  0<=x<=Len
...
%analytical solution
 ...
 TT(xx,tt) = symsum((40/((2*w-1)*pi)*sin((2*w-1)*pi/2) ...
   *cos((2*w-1)*pi/4*xx)*exp(-4*w^2*tt)), w, 1, Inf);
...
for i_time = 1:50:tnodes
    ...
    xlabel('x')                 % title of x axis
    ylabel('temperature')       % title of y axis
    pause(0.01)                  % sets time distance between two figures
end
%}
%{
